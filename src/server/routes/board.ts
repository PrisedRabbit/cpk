 ...
 */
function checkCoordinationVersion(projectId: string): { stale: boolean; file_version?: string; cli_version: string } {
  ...
  try {
    ...
    if (!firstLine.startsWith(COORDINATION_VERSION_PREFIX)) return { stale: true, cli_version: VERSION };
